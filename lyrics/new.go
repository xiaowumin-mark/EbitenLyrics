...
func (l *Line) LineAnimate(lyrics *Lyrics) {
	...
	for _, it := range l.BackgroundLines {
		it.AlphaAnimate = anim.NewTween(
			uuid.NewString(),
			time.Duration(500)*time.Millisecond,
			time.Duration(200)*time.Millisecond,
			1,
			it.Position.GetAlpha(),
			1.0,
			anim.EaseOut,
			func(value float64) {
				//e.Alpha = value
...
func (l *Line) FrameAnimate(lyrics *Lyrics) {
	...
	for elei, e := range l.OuterSyllableElements {
		kf := createFrames(l.OuterSyllableElements, elei, l.OuterSyllableElements[len(l.OuterSyllableElements)-1].EndTime-l.OuterSyllableElements[0].StartTime, 0.5)
		if e.Animate != nil {
			e.Animate.Cancel()
			e.Animate = nil
		}
